...
| 多个角色混在一起，分不清谁在决策 | 角色输出标注：每个角色带 `[ff] 角色名：` 标签，决策权归属清晰 |
...
[ff] 需求分析师：提取 8 个约束，核心是订单列表+筛选+分页
[ff] UI 设计师：拆为 3 个组件，列表用 ListView.builder
[ff] 架构设计师：订单状态用 Riverpod，全局用 Bloc
[ff] 页面工程师：开始生成代码，预计 3 个文件
```
...
|------|------|
| 轻量任务 | `[ff] 轻量任务，直接执行` |
| 页面开发 | `[flutter-forge] 模式：页面开发` |
| 代码审查 | `[flutter-forge] 模式：代码审查` |
| 迁移辅助 | `[flutter-forge] 模式：迁移辅助` |
| i18n/a11y 检查 | `[flutter-forge] 模式：i18n/a11y 检查` |
| 迭代中项目扫描 | `[flutter-forge] 模式：迭代中项目扫描` |
| 新项目初始化 | `[flutter-forge] 模式：新项目初始化` |
| 需求理解 | `[flutter-forge] 模式：需求理解 -> 设计包` |

`[ff]` 标记是必选输出，用户在连续对话中能随时感知 flutter-forge 是否在工作。
...
当前版本：**0.6.4** · [CHANGELOG](CHANGELOG.md)
