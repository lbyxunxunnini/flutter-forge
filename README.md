...
GitHub: [lbyxunxunnini/flutter-forge](https://github.com/lbyxunxunnini/flutter-forge) · License: MIT · 当前版本：0.5.1
...
- **运维直通**：只处理硬排除场景（非 Flutter 项目、纯知识问答、闲聊、通用 git、打包运行、环境搭建），不处理任何 Flutter 任务。
- **轻量任务**：改文案、颜色、字号、已定位 bug、analyze/test/build、依赖配置、CI 等，直接由页面工程师处理，只输出必要的 `[f-forge]` 标记。
...
```
任务进来
  → 是 Flutter 项目中的任务？ → 否 → 硬排除？ → 是 → 运维直通（退出 skill）
                                    → 否 → 正常对话处理
  → 从需求/设计开始？ → 是 → 完整流程
...
Flutter 项目中所有任务（包括 analyze/test/build、依赖配置、CI、文档）都由 flutter-forge 接管，按轻量/中等/完整流程处理。
...
| 架构级任务 | `[f-forge] 模式：架构级任务`（含代码审查、迁移、i18n/a11y、重构、依赖清理） |
...
当前版本：**0.5.2** · [CHANGELOG](CHANGELOG.md)
版本说明：本次 `0.5.2` 新增架构级任务分类（优化/重构/审查/迁移/依赖清理），运维直通重定义为仅覆盖排除场景，新增角色流程选择表和条件跳过，修复组件抽取标准矛盾和需求阶段重开冲突。
