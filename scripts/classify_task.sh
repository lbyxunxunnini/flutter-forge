...
case "$TASK_TEXT" in
  ff-fast) TASK_TEXT="" ;;
  ff-fast[[:space:]]*) TASK_TEXT="${TASK_TEXT#ff-fast}" ;;
  ff-a) TASK_TEXT="" ;;
  ff-a[[:space:]]*) TASK_TEXT="${TASK_TEXT#ff-a}" ;;
  ff[[:space:]]a) TASK_TEXT="" ;;
...
if echo "$stripped" | grep -qE '^ff-a([[:space:]]|$)|^ff[[:space:]]+a([[:space:]]|$)'; then
  policy="全自动"
elif echo "$stripped" | grep -qE '^ff-fast([[:space:]]|$)'; then
  policy="快速"
