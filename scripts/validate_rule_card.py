...
PLACEHOLDER_VALUES = {"", '""', "''", "new|existing", "low|medium|high"}
...
def parse_yaml_like(path: Path) -> tuple[dict[str, str], dict[str, int], list[str]]:
    ...
    for line_number, raw_line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        # Empty value means this key owns an indented mapping or sequence.
        if raw_value == "":
...
def main() -> int:
    ...
    parser.add_argument(
        "--allow-placeholders",
        action="store_true",
        help="Validate schema shape only; allow template placeholders and empty fields.",
    )
