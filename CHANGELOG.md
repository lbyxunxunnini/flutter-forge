...
0.7.2 是一次面向“新项目入口稳定性”和“文档瘦身”的整理版本，重点把新/老项目判断与规则卡流程解耦，并减少主入口与 README 的重复规则描述。
...
0.7.1 是一次把 H5 Forge 中已经验证过的“共创与确认闸门”同步回 Flutter Forge 的版本，重点补强新项目共创、确认粒度和 UI 输出节奏。
...
0.7.0 是一次开源准备和全接管路由升级版本，目标是在 Flutter 项目中兼顾能力覆盖和执行效率。

- 补充 MIT License、贡献指南、GitHub issue 模板和开源发布检查清单
- 修正 `.skillhub.json` 版本号，使其与 `VERSION` 和 README 的 `0.7.0` 保持一致
