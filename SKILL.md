...
| 中等 | 扫描→短判断→改动契约→确认/执行 | 实现前必须输出页面工程师的扫描结论、执行策略和改动契约；标准 `ff-` 下改动契约未获用户确认不得写代码；升级时输出新模式+原因+从对应阶段继续 | `[f-forge] 页面工程师：中等任务，先扫描后执行` + `[f-forge] 页面工程师：扫描结论：...` + `[f-forge] 页面工程师：执行策略：...` + `[f-forge] 页面工程师：改动契约：...` + 完成时 `[f-forge] 页面工程师：已完成修改` |
