...
```
任务进来
  → 是 Flutter 项目？ → 否 → 不命中 flutter-forge，退出
  → 能 10 秒内说清改什么？ → 是 → 直接改（轻量路径），不读后续内容
  → 涉及以下任一项？ → 否 → 不命中 flutter-forge，退出
    · 新页面 / 复用 / 组件边界 / 状态方案
    · 代码审查 / 迁移 / 国际化 / 无障碍
  → 是 → 确定工作模式 → flutter-forge 完整流程，继续读后续内容
```

### 快速退出条件

满足以下任一条件 → **不命中 flutter-forge**，直接用普通编码模式处理：

1. 不是 Flutter 项目（pubspec.yaml 不存在或不含 flutter SDK）
2. 纯 Dart 逻辑修改（不涉及 Widget / UI）
3. 只改配置、环境变量、CI 脚本
4. 纯知识问答（"Bloc 怎么用"、"Riverpod 和 Provider 区别"）
5. 闲聊、确认、追问等不涉及任务执行的对话
...
<!-- 以上是路由决策层。轻量任务和快速退出到这里就够了，不需要读后续内容。 -->
...
如果没有 session.md 或 session 中没有活跃任务 → 走正常任务路由。
