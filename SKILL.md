...
1. 检查规则卡？（首次接入/规则卡不存在）→ 自动初始化规则卡
2. 直通模式？（文档/环境/打包/CI/CD/闲聊/快速确认）→ 直通
3. 新项目共创？（模糊想法/空项目/讨论方向阶段）→ 新项目共创
4. 优化场景？→ 需求+优化→功能开发 / UI优化→UI优化任务 / 架构优化→架构级任务
5. 架构级任务？（代码审查/迁移/重构/依赖清理/i18n-a11y/性能优化）
6. 页面开发？（新页面/新模块/页面结构与路由接入）
7. 功能开发？（大功能闭环/跨页面状态联动/弹窗+提示栏+深链+业务流程）
8. 从需求或设计起步？→ 页面开发或功能开发（按任务语义）
9. 通过 10 秒测试？→ 轻量任务
10. 中间地带？→ 中等任务
11. 其他 → 功能开发
