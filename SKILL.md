...
|------|------|
| 轻量任务 | `[ff] 轻量任务，直接执行` |
| 新页面/模块开发 | `[flutter-forge] 模式：页面开发` |
| 代码审查 | `[flutter-forge] 模式：代码审查` |
| 迁移辅助 | `[flutter-forge] 模式：迁移辅助` |
| 国际化/无障碍检查 | `[flutter-forge] 模式：i18n/a11y 检查` |
...
直接执行，少解释。跳过启动握手和输入模型日志，只输出 `[ff] 轻量任务，直接执行`，然后立即读文件 → 修改 → 验证。
...
每个角色发言带 `[ff]` 标记（格式见 [skill_visibility.md](references/skill_visibility.md)），逐轮推进，最终输出结论行。
