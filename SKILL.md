...
2. 先路由 → 再分类 → 再展示模式 → 再执行（若路由判定为等待态，链路停在"路由"环，输出进入日志 + 等待态日志，不进入分类和模式展示）
3. 任务类型和任务规模分离（`大任务` 只代表复杂度）
4. `完整流程` 是内部执行协议（S1→S2→(S3)→S4→S5），对外以具体模式名呈现（功能开发/页面开发）。S3 为可选的角色对话/拆包阶段（仅大任务并行时需要）
...
选哪个？一句话：小改动用 `ff-fast`（先做再说，有风险再问），明确需求但缺细节用 `ff-a` 或 `ff a`（自动补全，做完告诉你）。
...
`等待态` 是内部等待状态，不属于用户可见模式名。行为约束：等待期间 controller 保持当前上下文不丢失；用户回复后直接从路由第 2 条重新判定（不重新走规则卡检查等完整启动流程）；如果用户长时间无回复（跨会话），下一次输入按新任务处理。
...
等待态追问后恢复时，从路由第 2 条重新判定（跳过规则卡检查，规则卡状态已在本次启动时确认）。
...
| 轻量 | 读→改→验证 | 不走握手，不拉专项 | `[f-forge] 页面工程师：轻量任务，直接执行` + 完成时 `[f-forge] 页面工程师：已完成修改并完成基本验证` |
| 中等 | 扫描→短判断→执行或升级 | 升级时输出新模式+原因+从对应阶段继续 | `[f-forge] 页面工程师：中等任务，先扫描后执行` + 完成时 `[f-forge] 页面工程师：已完成修改` |
...
命中触发词后、开始执行前，必须按以下顺序输出：

**第一步：输出进入日志**（命中触发词后立即输出）

```text
[f-forge] 进入 controller
```

**第二步：输出模式日志**（路由判定完成后立即输出，不等执行结果）
...
[f-forge] 模式：UI 优化
[f-forge] 模式：架构级任务
[f-forge] 模式：功能开发          ← 必须紧跟升级原因
[f-forge] 模式：页面开发          ← 必须紧跟升级原因
[f-forge] 模式：新项目共创        ← 必须紧跟升级原因
[f-forge] 模式：启动握手          ← 仅首次接管或规则卡异常时
[f-forge] 等待态：任务未明确，等待用户输入
```

`ff-fast` 启用时，在模式日志前先输出：
```text
...
**第三步：阶段变化时输出阶段日志**（每进入新阶段前必须先输出，否则不允许继续执行）
...
```text
[f-forge] 本轮完成：xxx（核心内容摘要）
```

轻量/直通任务只在开始和结束时各输出一次 `[f-forge]`，不展开阶段日志。
...
- 10 秒测试以用户输入为准
- 宿主能力未知时不假装并行
- 进入后必须先输出 `[f-forge] 进入 controller`，再输出 `[f-forge]` 模式日志，模式名必须使用 [skill_visibility.md](references/skill_visibility.md) 中定义的标准名称，禁止自创模式名
