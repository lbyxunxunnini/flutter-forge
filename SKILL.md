...
name: flutter-forge
description: >-
  Flutter 项目内的结构化协作主控 skill。用户消息以 ff-、ff-fast、ff-a、ff a、/flutter-forge 开头时进入；也可在用户明确要求按 Flutter 项目结构化流程处理时进入。命中后由 controller 在当前整轮任务内分流（直通/轻量/中等/UI 优化/架构级/功能开发/页面开发/新项目共创），完成后自动退出。完整触发词与匹配规则见 references/trigger_words.md。
---
...
**1. 任务路由分类**：运行 `scripts/classify_task.sh "<用户输入>"` 获取任务类型预判（直通/轻量/中等/UI 优化/架构级/功能开发/页面开发/新项目共创）和执行策略（标准/快速/全自动）。脚本失败时按下列顺序自行判定。
...
6. 属于优化场景吗？→ 需求优化进入功能开发/页面开发的 S1；UI 优化进入 UI 优化；架构优化进入架构级任务
7. 属于架构级重任务吗？→ 迁移/系统性重构/依赖治理/i18n-a11y 体系化接入/性能专项优化进入架构级任务
