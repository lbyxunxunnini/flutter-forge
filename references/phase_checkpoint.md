...
运行 `scripts/validate_output.sh` 校验当前会话累积的 `[f-forge]` 输出：
...
2. 如果 `等待状态 != none`，输出 `[f-forge] 主控：恢复等待，上一轮正在等待{等待输入类型}，已接入本轮补充并继续 {当前模式} / {当前阶段}`；否则输出 `[f-forge] 主控：恢复阶段，{当前阶段}` 告知用户
3. 从 `最近操作` 指示的位置继续执行
4. 等待态恢复并消费用户输入后，运行 `scripts/ff_session.sh consume-resume --user-input "<摘要>"`
5. 不重新走路由和分类
