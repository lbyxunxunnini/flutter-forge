...
在真正进入角色流转前，`flutter-forge` 应先输出一段很短的启动握手日志。下面是一份符合当前机制的示例。
...
- 项目状态：未初始化
- 当前判断来源：项目扫描
- Flutter skills：已检测到本地协作技能目录 /Users/agi00114/.claude/skills
- Flutter skills 状态：已就绪
- 当前模式：老项目扫描

[flutter-forge] 启动分流结果
- 分支：老项目 + 无规则卡
- 默认建议：先初始化项目规则
...
1. `flutter-forge` 已经介入
2. 当前判断这是新项目还是老项目
3. 有没有规则卡
4. 有没有可协作的 Flutter skills
5. 接下来会进入哪条分流
...
一旦正式规则卡生成完成，并且 Flutter skills 状态也已就绪，后续继续进入该项目时，默认不再重复输出这整段启动握手，而是直接进入当前任务阶段。只有在规则卡缺失、规则冲突、技能缺失、映射失效或用户明确要求时，才重新展开握手日志。
