...
- `flutter_skills_reminder_suppressed: true`

当用户明确表示不想下载官方 Flutter skills、也不想做本地映射时，写入该标记。之后只保留探测和状态输出，不再重复给安装或映射提醒。
...
规则卡一旦存在，也代表该项目已经被 Flutter Forge 接管。
...
- 该项目视为已初始化
- 该项目视为已被 Flutter Forge 接管
- 后续继续进入时，不再默认向用户重复输出完整启动握手

如果用户只是一次性小任务，且没有长期协作意图：
...
不要在用户选完模板后，又回退到“重新选择是否扫描老项目规则卡”的旧流程。
