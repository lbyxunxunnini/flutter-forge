...
- 运维直通：Flutter 项目内 analyze/test/build、依赖配置、文档、CI、已定位编译或测试失败，跳过完整启动握手，只输出 `[f-forge] 页面工程师：运维直通，快速执行`
...
运维直通不是退出 flutter-forge。它用于兼顾全接管和效率：保留 `[f-forge]` 状态感知，但不读取角色卡、不展开四角色、不做完整设计包。
