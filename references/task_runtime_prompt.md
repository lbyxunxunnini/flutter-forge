...
| S2→S3 或 S2→S4 | 方案稳定？上游无未收口？ | 进入 S3 输出 `[f-forge] 阶段：S3 拆包冻结`；直接进入 S4 输出 `[f-forge] 阶段：S4 实现中` | **S3 可跳过**：首批范围足够小（1-2 个工作包，无需并行）或非大任务时直跳 S4。**S2→S4 是硬阻断**：S2 方案确认完成后，必须在下一条回复中输出 `[f-forge] 阶段：S4 实现中` 并开始实现，不允许在 S2 输出"结论"后退出或等待。若用户追加补充信息，视为"已确认项的补充"直接纳入方案，不重新进入 S1/S2 分析 |
...
每次阶段切换时，必须通过 `scripts/ff_session.sh` 将当前状态写入宿主 session。session 路径和格式以 [session_management.md](session_management.md) 为准：
