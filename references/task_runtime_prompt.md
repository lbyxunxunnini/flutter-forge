...
- 中等任务扫描结论和执行策略中，每条改动点必须指明具体文件路径和行为变化。禁止"添加适当的错误处理""优化相关逻辑"等模糊表述。发现无法具体化时，升级为功能开发/页面开发从 S2 重新收口
...
多工作包执行期间，controller 必须连续执行所有已冻结的工作包，不得在工作包之间暂停询问用户"是否继续"。

- S3 拆包冻结后，所有工作包按顺序（串行）或并行执行，中间不暂停
- 每个工作包完成时输出结果摘要日志，但不等待用户确认
- 只有以下情况允许在工作包间暂停：
  1. 当前工作包发现共享约束被破坏
  2. 上游方案变化导致后续工作包失效
  3. 命中高风险中断条件（ff-a 下）
- S5 统一收口验证是唯一允许的整体暂停点
...
- 若是等待态恢复，先输出 `[f-forge] 主控：恢复等待，...`，随后运行 `scripts/ff_session.sh consume-resume --user-input "<摘要>"` 清空等待态
- 不要重新走普通问答路径
