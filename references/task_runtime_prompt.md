...
2. 运行 `scripts/classify_task.sh "<用户输入>"` 获取任务类型预判和执行策略，LLM 消费预判结果（`confidence: low` 时做二次判定）。脚本不存在或执行失败时，LLM 按 `SKILL.md` 路由顺序自行判定，不因脚本缺失阻塞启动
