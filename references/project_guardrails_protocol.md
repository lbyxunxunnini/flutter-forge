# Flutter Forge Reference - Project Guardrails 协议

Project guardrails 是 flutter-forge 的项目锚点与长期护栏文件，定义了项目状态检测、护栏存储、检查和来源判定。

## 项目根状态模型

flutter-forge 使用三态项目根分类：

1. `flutter_existing` — 非空目录，有 `pubspec.yaml` 和 `lib/`，flutter-forge 可介入
2. `non_flutter` — 非空目录，不满足 Flutter 项目结构，flutter-forge 不介入
3. `empty_new` — 空目录（排除隐藏文件），视为新项目，优先级高于 Flutter 检测
...
1. `.claude/.flutter-forge/projects/*.project_guardrails.yaml`
2. `.trae/.flutter-forge/projects/*.project_guardrails.yaml`
3. `.agents/.flutter-forge/projects/*.project_guardrails.yaml`
4. `.flutter-forge/projects/*.project_guardrails.yaml`
...
- 如果前三个目录都不存在或都没有当前项目 guardrails，则回退到项目根目录下的 `.flutter-forge`
- 当需要初始化正式 guardrails 时，按优先级在第一个存在的目录中创建；如果都不存在，优先在 `.claude/.flutter-forge/projects/` 创建
...
```text
status: found | missing | empty_new | non_flutter
path: <相对路径或 ->
project_name: <项目名>
root_type: flutter_existing | non_flutter | empty_new
forge_enabled: true | false
```

- `status: found` → 加载 `path` 指向的正式 guardrails
- `status: missing` → `flutter_existing` 项目但无 guardrails，进入初始化流程
- `status: empty_new` → 空目录新项目，进入共创流程
- `status: non_flutter` → 非 Flutter 工作区，flutter-forge 不介入
...
2. 按下列顺序读取项目根目录下的 guardrails，命中第一个存在的即停：
   - `.claude/.flutter-forge/projects/*.project_guardrails.yaml`
   - `.trae/.flutter-forge/projects/*.project_guardrails.yaml`
   - `.agents/.flutter-forge/projects/*.project_guardrails.yaml`
   - `.flutter-forge/projects/*.project_guardrails.yaml`
...
- 有 guardrails：说明该项目已经被 Flutter Forge 初始化过
...
如果当前项目不存在真实 guardrails（`flutter_existing` 状态）：
...
- .claude/.flutter-forge/projects/<project>.project_guardrails.yaml
...
除了 Flutter Forge 自己的 guardrails，还要扫描项目中已有的规则文件：
...
- `status: non_flutter`：flutter-forge 不介入
